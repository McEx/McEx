defmodule McEx.Entity.Property.Physics do
  ...
  def handle_world_event(:entity_tick, _, state) do
    ...
    area_block_positions = AABB.from_points(position.pos, Vec3D.add(position.pos, velocity))
    |> AABB.blocks_in
    |> Enum.take(64)

    area_blocks = area_block_positions
    |> Enum.map(fn pos -> {AABB.block_aabb(pos), get_block(pos, state)} end)

    entity_aabb = AABB.from_pos_size(position.pos, prop.size)
    movement_delta = do_collision(entity_aabb, velocity, area_blocks)
