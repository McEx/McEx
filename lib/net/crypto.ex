...
defmodule McEx.Net.Crypto do
  ...
  def encrypt(plaintext, %CryptData{} = cryptdata), do: encrypt(plaintext, cryptdata, <<>>)
  def encrypt(<<plain_byte::binary-size(1), plain_rest::binary>>, %CryptData{key: key, ivec: ivec} = cryptdata, ciph_base) do
  ...
  def encrypt(<<>>, %CryptData{} = cryptdata, ciphertext) do
    {cryptdata, ciphertext}
  end

  def decrypt(ciphertext, %CryptData{} = cryptdata), do: decrypt(ciphertext, cryptdata, <<>>)
  def decrypt(<<ciph_byte::binary-size(1), ciph_rest::binary>> = ciph, %CryptData{key: key, ivec: ivec} = cryptdata, plain_base) do
  ...
  def decrypt(<<>>, %CryptData{} = cryptdata, plaintext) do
    {cryptdata, plaintext}
  end

  def update_ivec(ivec, data) when byte_size(data) == 1 and byte_size(ivec) == 16 do
    <<_::binary-size(1), ivec_end::binary-size(15)>> = ivec
    <<ivec_end::binary, data::binary>>
  end

  def stupid_sha1(data) do
